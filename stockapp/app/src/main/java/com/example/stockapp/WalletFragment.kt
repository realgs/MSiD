...
class WalletFragment : Fragment() {
  ...
  private fun setAddWalletButton(view: View) {
    val button =
      view.findViewById<com.google.android.material.floatingactionbutton.FloatingActionButton>(R.id.addWallet)
    ...
    button.setOnClickListener {
      ...
      builder.setPositiveButton(
        ...
        run {
          ...
          val stockName =
            mView.findViewById<Spinner>(R.id.wallet_dialog_spinner).selectedItem.toString()
          if (walletName.isNotEmpty() && !Globals.wallets.map { it.value.name }
              .contains(walletName.toString())) {
  ...
  private suspend fun updateWorthLabel(view: View) {
    while (true) {
      ...
      view.findViewById<TextView>(R.id.walletWorth).text =
        String.format("%.${decimalPlaces}f", newVal)
    }
  }
}
